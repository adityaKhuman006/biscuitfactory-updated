...
<div class="main-panel">
    <div class="content-wrapper">
        <div class="row">
            <div class="card p-2">
                <div class="col-12">
                    <h3 class="fw-bold p-2">Biscuit Master</h3>
                    <table class="table table-hover tab-pane">
                        ...
                        @foreach ($account as $key => $item)
                            <tr class="removeble-tr-{{ $item->id }}">
                                ...
                                <td>
                                    <img src="{{ asset('public/img/' . $item->img) }}" data-id="{{ $item->id }}"
                                        alt="Biscuit Image" class="" />
                                    <input type="file" class="update-img" data-id="{{ $item->id }}" />
                                </td>
                                <td>
                    ...
                    <div class="card-body pt-10 ">
                        ...
                        <form method="POST" action="{{ route('master.biscuite.create') }}"
                            ...
                            <div class="repeater">
                                        ...
                                        <div class="row mt-3" data-repeater-item>
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label>Name</label>
                                                    <input type="text" name="category-group[][name]" required
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label>Images</label>
                                                    <input type="file" name="category-group[][img]" required
                                                        class="form-control form-control-sm" placeholder="Upload Image">
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="form-group">
                                ...
                                <div class="text-end">
                                    <button data-repeater-create type="button" class="btn-sm btn btn-success add-item"
                                        style="margin-top: 15px;"><i class="mdi mdi-plus"></i></button>
                                </div>
                            </div>
...
<script>
    ...
    $(document).ready(function() {
        $(".update-name").change(function() {
            ...
            $.ajax({
                url: '{{ route('update.biscuite.name') }}',
                method: 'POST',
                data: {
                    id: id,
                    name: name,
    ...
    $("#deleteItem").click(function() {
        ...
        var id = $(this).attr('data-id');
        $.ajax({
            url: '{{ route('delete.biscuite.master') }}',
            method: 'POST',
            data: {
                id: id,
                _token: '{{ csrf_token() }}'
